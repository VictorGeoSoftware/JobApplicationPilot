...
- **Lever job boards (public API, no auth):** `GET https://api.lever.co/v0/postings/{company}` — plain JSON, no special UA needed. Slug = the `{company}` in `jobs.lever.co/{company}` / `jobs.eu.lever.co/{company}` URLs (note: some slugs 404 even when the hosted board works — Finom's is `pnlfin` on jobs.eu.lever.co but the API rejects it; fall back to scraping the hosted board HTML via Tavily).
...
- 2026-08-20 (run 3): **Country lists vary per posting at the same company** — ElevenLabs now has Sweden-eligible eng roles (FDE-Sweden, Internal AI Transformation, ElevenAPI, FE-leaning Full-Stack) while its generic Full-Stack posting still excludes Sweden; verify each posting ID, don't generalize a company verdict. Cohere uses Ashby with generic `"Europe"` secondary locations (no country list → ❓, not ✅). When LinkedIn shows "No longer accepting applications", check the company's own ATS board — Finom's Senior AI Engineer is closed on LinkedIn but live on its Lever board. Added Lever public API as a source (with slug-404 caveat). Remotive/Arbeitnow APIs yielded nothing AI-relevant this run.
